...
class ChatLogActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
       ...
       // val username = intent.getStringExtra(NewMessageActivity.USER_KEY)
        ...
        send_chat.setOnClickListener {
            Log.d(Tag, "Attempt to send $text")
            performSendMessage()
    ...
    private fun performSendMessage(){
        ...
        text = msg_chat_log.text.toString()  // edit text from activity_chat_log
        val fromID = FirebaseAuth.getInstance().uid
        ...
        if (text!!.isNotEmpty()){
            chatMessage = ChatMessage(references.key!!, text!!, toID, fromID, System.currentTimeMillis() / 1000)
        }else{
            return
        }
